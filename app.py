...
for coluna in colunas\_necessarias:
...
df\["Valor"\] \= pd.to\_numeric(  
    df\["Valor"\],  
    errors="coerce"  
).fillna(0)

df\["Quilometragem"\] \= pd.to\_numeric(  
    df\["Quilometragem"\],  
    errors="coerce"  
).fillna(0)
...
st.sidebar.markdown("\<br\>", unsafe\_allow\_html=True)
...
style="background-image: url('{IMAGEM\_HERO}');"\>
...
""",  
    ...
    total\_carros \= len(df)

    valor\_total \= df\["Valor"\].sum()

    km\_total \= df\["Quilometragem"\].sum()
...
""",  
    ...
    with st.form(  
        ...
        with col1:
            ...
            marca \= st.text\_input(  
                "🏷️ Marca"  
            )

            modelo \= st.text\_input(  
                "🚗 Modelo"  
            )

            ano \= st.number\_input(  
                "📅 Ano",  
                min\_value=1900,  
                max\_value=2035,  
                value=2024,  
        ...
        with col2:
            ...
            placa \= st.text\_input(  
                "🔢 Placa"  
            )

            quilometragem \= st.number\_input(  
                "🛣️ Quilometragem",  
                min\_value=0,  
                value=0,  
                step=100  
            ...
            observacoes \= st.text\_area(  
                "📝 Observações"  
            )

        cadastrar \= st.form\_submit\_button(  
            "💾 CADASTRAR VEÍCULO"  
        )
...
""",  
        ...
        opcoes\_carros \= df.index.tolist()

        carro\_excluir \= st.selectbox(  
            "🗑️ Selecione um veículo para excluir",  
            options=opcoes\_carros,  
            format\_func=lambda indice:  
